...
//~ 		1, "I", 2, "II", 3, "III", 4, "IV", 5, "V", 6, "VI", 7, "VII", 8, "VIII", 9, "IX", 10, "X");
	...
	private final String presentPassiveForm;
	...
	public static class Builder {
		private final int numBucknell;
		private final String rootName;
		private final List<Integer> rootClass = new ArrayList<>(2);
		private final String meaning;
		private final Map<Pada, String> citationForm = new EnumMap<>(Pada.class);
		private String presentPluralForm = "";
		private String presentPassiveForm = "";
		private List<String> futureForm = new ArrayList<>();
		...
		public Builder addRootClass(final int num) {
			rootClass.add(num);
			return this;
		}
	...
	public String getCitationForm() {
		final String result = citationForm.getOrDefault(Pada.ACT, citationForm.get(Pada.MID));
	...
	public String getDescription() {
		final String desc = rootName + " " + getRootClassStr() + " " + meaning +" [" + numBucknell + "]";
		return desc;
	}

	public String getFullDescription() {
		final String desc = getCitationForm() + " < " + rootName + " " + getRootClassStr() + " " + meaning +" [" + numBucknell + "]";
		return desc;
	}

	public String getParadigmName(final TenseMood tense, final Pada pad) {
	...
	public Map<Person, Map<Number, List<String>>> getCommonPassiveProduct(final TenseMood tense) {
		final String paradName = presentPassiveForm.isEmpty()
									? ""
									: SktConjugation.getParadigmName(presentPassiveForm, tense, Pada.MID);
		return paradName.isEmpty()
				? Collections.emptyMap()
				: SktConjugation.compute(presentPassiveForm, paradName);
	}
	...
	public Map<Person, Map<Number, List<String>>> getPerfectProduct(final Pada pad) {
		...
		if (subList == null)
			return SktConjugation.compute(getPerfectForm(), paradName); 
		else
			return SktConjugation.computePerfect(getPerfectForm(), paradName, subList); 
	}
	...
	public Map<Person, Map<Number, List<String>>> getPrecativeProduct(final Pada pad) {
		...
		final String subForm = pad == Pada.ACT
							? !presentPassiveForm.isEmpty() ? presentPassiveForm : ""
	...
	public Map<Case, Map<SktDeclension.Number, List<String>>> getPresentPassiveParticipleProduct(final Gender gend) {
		if (presentPassiveForm.isEmpty())
			return Collections.emptyMap();
		final String partStem = Sandhi.applyInternalSandhiN(presentPassiveForm.substring(0, presentPassiveForm.length()-3), "amāna");
		final NominalParadigm parad = SktDeclension.paradigmMap.get(NominalParadigm.genericFallback[gend.ordinal()]);
		return SktDeclension.compute(new SktNominal(partStem, parad));
	}
	...
	public Map<Case, Map<SktDeclension.Number, List<String>>> getPerfectActiveParticipleFromTaProduct(final Gender gend) {
		if (pppForm.isEmpty())
			return Collections.emptyMap();
		final String perfPartStem = pppForm + "vat";
		final SktNominal.Adjective adj = new SktNominal.Adjective(perfPartStem);
		final String[] terms = adj.getTerms();
		final String[] paradNames = adj.getParadigmNames();
		final NominalParadigm parad = SktDeclension.paradigmMap.get(paradNames[gend.ordinal()]);
	...
	public Map<Case, Map<SktDeclension.Number, List<String>>> getPerfectPassiveParticipleProduct(final Gender gend) {
		if (pppForm.isEmpty())
			return Collections.emptyMap();
		final NominalParadigm parad = SktDeclension.paradigmMap.get(NominalParadigm.genericFallback[gend.ordinal()]);
		return SktDeclension.compute(new SktNominal(pppForm, parad));
	}
	...
	public List<StringPair> getAbsolutiveProduct() {
		...
		final String prefAbs = !prefixedAbsolutiveForm.isEmpty()
								...
								: !presentPassiveForm.isEmpty()
									? "-" + presentPassiveForm.substring(0, presentPassiveForm.length()-2)
									: "";
		if (!prefAbs.isEmpty()) {
